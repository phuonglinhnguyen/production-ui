...
export default class FormWrapper extends Component {
    ...
    if (task.record_select_waiting > -1) {
      self.__validationFieldsOfRecord(() => {
        self.__validationSectionOfRecord(() => {
          self.setState({ focusError: { record: task.record_select_waiting, fieldName: fields[0].name, time: Date.now() } })
  ...
  handleKeyDown = (event) => {
    ...
    if (event.altKey) {
      ...
      try {
        disable_update_record = layout.item.section.settings.multiple.mask === "require_mask"
      } catch (error) { }
    ...
    if (event.altKey && key_code === 'm') {
      ...
      else if (event.altKey && key_code === 'b') {
  ...
  __validationSectionOfRecord = (callback) => {
    const { section_visiable } = this.props
    const {
      task,
      layout,
      notifyActions,
    ...
    if (section_visiable && section_visiable.length > 0) {
      let section_name = section_visiable;
      ...
      if (sectionError.has(task.record_selecting) &&
        ...
        taskActions.setNextRecordWaiting(-1)
  ...
  checkValidationChangeRecord = (isNext) => {
    const self = this;
    const {
      task,
      layout,
  ...
  handleBackRecord = () => {
    this.checkValidationChangeRecord(false);
  }
  ...
  hanbleCloseDialogConformSubmitClick = () => {
    const { layout, task, fields } = this.props;
