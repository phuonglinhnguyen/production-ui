...
export const getDatasKeying = (records, sectionName, layout, userName, task, source, location, reason) => {
  ...
  let fields_mixed =section_mixed.fields; 
  ...
  let result = records.map(_data => {
    ...
    let _data_value = {};
     let section_current =  layout.item.sections.filter(item => item.name === _data[field_control])[0]
    let fields = fields_mixed.concat(section_current.fields);   
...
export const getDatasKeyingTraining = (records, sectionName, layout, userName, task,source,location , reason) => {
  let section_mixed= layout.item.sections.filter(item => item.settings.type === 'mixed')[0];
  let field_control = section_mixed.settings.field_control
  let fields_mixed =section_mixed.fields; 
  let result = records.map(_data => {
