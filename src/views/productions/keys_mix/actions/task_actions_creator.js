...
const getDataOfTask = async (task, info, isGetTask) => {
  ...
  try {
    ...
    let _holdCount = (hold_count && hold_count.value)
      ...
      lastRecord =lastRecord.length===0?[{}]:lastRecord;
    return {
      records: lastRecord,
  ...
  try {
    if (!task.isFetching) {
      ...
      if (_res.error) {
        ...
        let redirectUrl = `${info.isTraining ? "/training" : ""}/${URL_MODULE}/${info.projectId}/${info.layoutName}/${info.taskId}`;
        info.history.push(redirectUrl);
      } else {
        let _task = _res.payload;
        let _datas = await getDataOfTask(_task, info);
        dispatch(claimReceive(_datas));
        let redirectUrl = `${info.isTraining ? "/training" : ""}/${URL_MODULE}/${info.projectId}/${info.layoutName}/${info.taskId}/${_datas.task.id}`;
        info.history.push(redirectUrl);
      }
    }
  } catch (e) {
...
const completeTask = (info, task, isNext, dataKeying, reason, is_define_mask) => async (dispatch) => {
  ...
  if (_res.error) {
    ...
    if (isNext) {
      ...
      dispatch(resetTask())
      info.history.push(
        `${info.isTraining ? "/training" : ""}/${URL_MODULE}/${info.projectId}/${info.layoutName}/${info.taskId}`
      );
    }
  }
};
