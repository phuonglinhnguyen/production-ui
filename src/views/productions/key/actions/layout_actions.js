...
const fetchLayout = (projectId, layoutName, sectionName, didInvalidate) => async (dispatch) => {
    try {
        ...
        // let _res = await XClient.field.list_of_section_by_layout(projectId, layoutName, sectionName);
        // let _resSection = await XClient.section.get_section_by_name(projectId, sectionName);
        // let section = getReferSetting(_resSection.payload, sectionName);
        if (_res.error) {
