  ...
  try {
    ...
    if (Array.isArray(data)) {
      data.forEach(function(element) {
        let id, s2_url, doc_name,batch_name;
        try {
...
export const saveTask = (
  ...
  if (is_layout_empty) {
    return dispatch({
      ...
      status_text:
        "productions.classify.each_document_must_be_classified_with_a_layout"
    });
  }
  ...
  try {
    ...
    if (!next_task) {
      history.push(
        `/classifying/${doc_size}/true/${project_id}/0/0/${task_def_key}`
      );
